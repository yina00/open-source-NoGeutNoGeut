...
function openChatRoom(roomNum) {
    ...
    // AJAX 또는 fetch를 사용하여 해당 채팅방의 메시지를 불러옵니다.
    fetch(`/chat/seniorRoom/${roomNum}`)
        .then(response => response.json())
        .then(data => {
            console.log("Received data:", data);  // 전체 데이터 출력
            const seniorName = data.seniorName;  // 서버에서 받은 seniorName을 사용
            console.log("Fetched seniorName:", seniorName);
...
async function send() {
    ...
    if (currentRoom) {
        ...
        try {
            const response = await fetch(`/chat/seniorRoom/${roomNum}`, {
                method: 'POST',
                headers: {
...
socket.on("promise", (data) => {
    ...
    console.log(`PromiseSender: ${sender} Time: ${time}, RoomNum: ${roomNum}, promise: ${promise}, promiseTitle: ${promiseTitle}`); // 로그로 각 값 확인
